...
1. **Regras (obrigatório)** — no Console, Firestore → **Segurança**, cola o ficheiro `firestore.rules` deste repositório e clica **Publicar**. As regras da screenshot (cliente não escreve ouro) estão certas; só falta publicar se o botão ainda estiver ativo.
